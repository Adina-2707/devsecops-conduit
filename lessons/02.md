...
**3. Your SSH key.** Most of you already have one. First ask GitHub whether it knows you:

```bash
ssh -T git@github.com
```

Type `yes` if it asks about the fingerprint. If the answer is
`Hi <your-name>! You've successfully authenticated`, your key is ready: go to step 4.

Otherwise, look for a key on this laptop:

```bash
ls ~/.ssh/*.pub
```

- You see `id_ed25519.pub` or `id_rsa.pub`: you have a key. Do not make a new one.
- You see `No such file or directory`: make one, pressing Enter three times
  (default file, no passphrase):
...
> [!WARNING]
> If `ssh-keygen` asks `Overwrite (y/n)?`, answer `n`. You already have a key, and
> replacing it locks you out of every place that trusts the old one.

Show the public half of your key (use `id_rsa.pub` if that is the one you have):

```bash
cat ~/.ssh/id_ed25519.pub
```

Copy the whole line. On GitHub open **Settings → SSH and GPG keys → New SSH key**,
paste it, click **Add SSH key**. Test again:
...
**C. Integration tests in CI.** On your laptop the integration tests needed a running
API and a database. The runner has neither. Add a job `integration-tests` to `ci.yml`
that ends with `python -m pytest tests/integration -v` and turns green. To get
there, your job must:

1. start PostgreSQL next to the job
   (read [PostgreSQL service containers](https://docs.github.com/en/actions/tutorials/use-containerized-services/create-postgresql-service-containers));
2. give the API its settings as environment variables, with the same names as in
   `backend/.env.example`;
3. install the packages of the API (`backend/requirements.txt`) and of the tests;
4. prepare the database, the same way `scripts/run-class.sh` does it;
5. start the API in the background and wait until `/api/health-check` answers;
6. only then run the tests.

<details>
<summary>Hint 1 · Where do the settings come from?</summary>

Put them under `env:` of the job. The API needs `APP_ENV`, `SECRET_KEY`,
`JWT_SECRET_KEY` and five `POSTGRES_*` values. The service needs the same user,
password and database name. These values are not secrets: the database lives for
one minute on a machine that is thrown away.

</details>

<details>
<summary>Hint 2 · How do I prepare the database?</summary>

Inside the `backend` folder: `python -m alembic -c alembic.ini upgrade head`.
A step can run in a folder with `working-directory: backend`.

</details>

<details>
<summary>Hint 3 · How do I start the API and wait for it?</summary>

`&` at the end of a command leaves it running in the background. Then wait:

```bash
timeout 30 bash -c 'until curl -fs http://127.0.0.1:8000/api/health-check; do sleep 1; done'
```

If the job fails, a last step with `if: failure()` that prints the API log helps
you find out why.

</details>

Checkpoint: **integration-tests** is green with `2 passed`. Compare its time with
**unit-tests**: the wider the test, the more it needs and the slower it is.

**D. Get ready for the next lesson: Azure for Students.** Next time you get your own
Linux server on the internet. Azure for Students pays for it: $100 of credit for 12
months, no credit card, only your college email.
...
2. Sign in with your **college email**. If Microsoft does not know this address yet,
   create a Microsoft account with it.
...
Do not create any servers yet. The credit is real money, and next lesson we create
exactly one server together. If the sign-up does not accept your email, do not use a
card; tell the instructor.
...
2. **`Permission denied (publickey)`.** The key on GitHub is not the one in
   your `~/.ssh/*.pub` file. Compare the end of both lines.
